...
Once enabled, review/chat sessions are filed under an auto-registered `GithubReviewer` workspace. Timing: at activation the plugin first checks the composition (via the loader) for the `workspace` service — if it is declared, the directory is created immediately and registration is retried until the service appears (its init can be slow); it is also re-checked **before every PR session creation**, guaranteeing the directory exists and the record is registered whenever the service is up. Compositions without the service get no directory or registration work. Adjust with `workspaceDir` / `workspaceTitle`. Pre-existing sessions stay in their old workspace; only new sessions use the new directory.
