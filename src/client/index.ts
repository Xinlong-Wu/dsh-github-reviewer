...
import githubReviewerRemote from 'dsh-github-reviewer/remote'
...
/** Register the Remote contribution, locale, styles, controller, and keyed card slot. */
export async function apply(ctx: ClientContext): Promise<void> {
  const disposeRemote = await ctx.remote.$mount(githubReviewerRemote)
  ctx.effect(() => disposeRemote, 'github-reviewer repository catalog Remote')
  const scope = ctx.settingsScope.bind<ReviewerSettings>({ namespace: SETTINGS_NAMESPACE })
  const { api } = ctx.get('connection') as ConnectionHandle
  const controller = new GithubReviewerCardController(scope, api, async (signal) => {
    const result = await ctx.remote.githubReviewerCatalog.repositories(signal)
